        ...
        try {
            ...
            ugi.doAs(new PrivilegedExceptionAction<Void>() {
                public Void run() throws Exception {
                    ...
                    final FileSystem fs = HdfsService.getFileSystem(hdfsUrls, conf);
                    for (String file : files) {
                        importFile(fs, file, partitionColumns, inputFormat, serDe, serDeParameters, hdfsUrls, hdfsUser, columns, outputColumns, useKerberos, ctx);
                    }
                    return null;
                }
            });
